<script lang="ts">
    let { placeholder = "", input=$bindable(null), icon = $bindable(null), value = $bindable(""), className = "", noDecoration = false, password = $bindable(false), onIconClick=() => {}, errorMessage = $bindable(""), textarea = false, maxLength = null, rows = 3, onSubmit = () => {} } = $props();
</script>
