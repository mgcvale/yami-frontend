<script lang="ts">
    ...
    let { 
        ...
        data: { likedReviews: PageableEntry<FoodReview>, user: CurrentUser }
    } = $props();

    const { number, size, numberOfElements, totalElements, totalPages, first, last } = data.likedReviews;
...
{#if data.likedReviews.empty}
    <p class="text-center my-4">Não foi encontrada nenhuma avaliação curtida por você.</p>
{:else}
    <div class="space-y-4 w-full">
        {#each data.likedReviews.content as entry }
            <FoodReviewCard review={entry} ownsReview={entry.userId === data.user.id} />
        {/each}
    </div>
