<script lang="ts">
    ...
    let { data }: { data: { user: PublicUser; stats: ReviewStats; userId: number; viewingSelf: boolean, followers: PublicUserWithoutCounts[] } } = $props();

    let searchQuery = $state("");
    let actualFollowers = $state(data.followers);
    ...
    $effect(() => {
        actualFollowers = data.followers.filter(datapoint => {
